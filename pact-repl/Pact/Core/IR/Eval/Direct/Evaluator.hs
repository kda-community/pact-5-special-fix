  ...
  evaluate' = \case
    ...
    BuiltinForm c info -> case c of
      ...
      CEnforce cond str -> do
        pact53Disabled <- isExecutionFlagSet FlagDisablePact53
        let env' = if not pact53Disabled then readOnlyEnv env else sysOnlyEnv env
...
evalWithStackFrame :: (IsBuiltin b, CoverageTick i e) => i -> StackFrame i -> Maybe Type -> DirectEnv e b i -> EvalTerm b i -> EvalM e b i (EvalValue e b i)
